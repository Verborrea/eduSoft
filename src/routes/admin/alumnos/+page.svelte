...
<div class="main">
		...
		<div class="fc g8">
			...
			<form method="post" use:enhance={() => {
				return async ({ update }) => {
					await update();
					ids.forEach(id => {
						const idx = data.alumnos.findIndex(a => a.id === id)
						data.alumnos[idx].status = true
					});
					ids = []
				};
			}}>
				<input class="none" type="text" name="ids" value={ids}>
				<button class="btn green" formaction="?/reactivate" disabled={ids.length === 0}>
					Reactivar
				</button>
			</form>
			<form method="post" use:enhance={() => {
				return async ({ update }) => {
					...
					ids.forEach(id => {
						const idx = data.alumnos.findIndex(a => a.id === id)
						data.alumnos[idx].status = false
					});
					ids = []
				};
			}}>
				<input class="none" type="text" name="ids" value={ids}>
				<button class="btn red" formaction="?/deactivate" disabled={ids.length === 0}>
					Dar de Baja
					<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
	...
	<div class="justify-between fc">
		<div class="fc g24">	
			...
			<span class="text-blue">{ids.length} estudiantes seleccionados</span>
		</div>
		<div class="fc g8">
