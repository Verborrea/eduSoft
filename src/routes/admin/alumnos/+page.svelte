<script lang="ts">
	...
	function selectAll() {
		if (selection.length < data.students.length) {
			selection = data.students.map((s) => s.id);
		} else {
			selection = [];
		}
	}
...
<div class="main">
	...
	<div class="fc g16">
		...
		<div class="fc g8">
			<button type="button" class="btn btn-secondary" disabled={selection.length === 0}>
				Asignar Grupos
				<svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="6" cy="17" r="3" stroke="currentColor" stroke-width="2"/><circle cx="18" cy="17" r="3" stroke="currentColor" stroke-width="2"/><circle cx="12" cy="7" r="3" stroke="currentColor" stroke-width="2"/></svg>
			</button>
			<form method="post" action="?/reactivate">
				<input class="none" type="text" name="ids" value={selection}>
				<button class="btn green" type="submit" disabled={selection.length === 0}>
					Reactivar
				</button>
			</form>
	...
	<table>
				...
				<th>
					<label>
						<input
							type="checkbox"
							class="checkbox"
							checked={selection.length === data.students.length}
							on:click={selectAll}
						/>
					</label>
				</th>
		...
		<tbody>
			{#each data.students as alumno}
				<tr class:selected={selection.includes(alumno.id)}>
					<td class="center">
						<label>
							<input
								type="checkbox"
					...
					<td><div class="cell-fc">
						<div
							class="status"
							class:active={alumno.status}
							class:inactive={!alumno.status}
						></div>
					</div></td>
					<td>
						<div class="cell-fc">
	...
	<footer class="justify-between fc g16">
		...
		<p class="text-blue">{selection.length} estudiante{selection.length === 1 ? '': 's'} seleccionado{selection.length === 1 ? '': 's'}</p>
		<div class="fc g16">
			<form>
				<button
