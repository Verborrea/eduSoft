<script lang="ts">
	...
	function selectAll() {
		if (selection.length < data.groups.length) {
			selection = data.groups.map((g) => g.id);
		} else {
			selection = [];
		}
	}
...
<div class="main">
	<header class="fc justify-between">
		...
		<a href="/admin/grupos/añadir" class="btn btn-secondary">
	...
	<div class="justify-between fc g16">
		<form id="filters" class="fc g16">
			...
			<select id="period" name="period" class="select" bind:value={period}>
				<option value="" selected>Todas los periodos</option>
				{#each data.periods as period}
					<option value={period}>{period}</option>
				{/each}
			</select>
			<select id="course" name="course" class="select" bind:value={course}>
	...
	<table>
				...
				<th>
					<label>
						<input
							type="checkbox"
							class="checkbox"
							checked={selection.length === data.groups.length}
							on:click={selectAll}
						/>
					</label>
				</th>
		...
		<tbody>
			{#each data.groups as group (group.id)}
				<tr class="hover" class:selected={selection.includes(group.id)}>
					<td class="center">
						<label>
							<input
								type="checkbox"
					...
					<td class="nowrap">{group?.expand?.course.name}</td>
					<td><div class="cell-fc">{group.turno}</div></td>
					<td><div class="cell-fc">{group.seccion}</div></td>
					<td><div class="cell-fc">{group.is_virtual ? 'Virtual' : 'Presencial'}</div></td>
					<td><div class="cell-fc">{group?.expand?.course.module}</div></td>
					<td>{group.horario}</td>
					<td><div class="cell-fc">{group.period}</div></td>
					<td>
						<div class="cell-fc">
							<a href="/admin/grupos/editar?id={group.id}">
								<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
	...
	<footer class="justify-between fc g16">
		...
		<p class="text-blue">{selection.length} grupo{selection.length === 1 ? '': 's'} seleccionado{selection.length === 1 ? '': 's'}</p>
		<div class="fc g16">
			<form>
				<button
