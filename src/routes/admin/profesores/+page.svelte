<script lang="ts">
	...
	function selectAll() {
		if (selection.length < data.teachers.length) {
			selection = data.teachers.map((t) => t.id);
		} else {
			selection = [];
		}
	}
...
<svelte:head>
  	<title>eduSoft | Profesores</title>
</svelte:head>

<div class="main">
	<header class="fc">
		<h1>Lista de Profesores</h1>
		<a href="/admin/alumnos/matricula" class="btn btn-primary">Añadir Profesor</a>
	</header>
	<form class="fc g16">
	...
	<table>
				...
				<th>
					<label>
						<input
							type="checkbox"
							class="checkbox"
							checked={selection.length === data.teachers.length}
							on:click={selectAll}
						/>
					</label>
				</th>
		...
		<tbody>
			{#each data.teachers as teacher}
				<tr class:selected={selection.includes(teacher.id)}>
					<td class="center">
						<label>
							<input
								type="checkbox"
					...
					<td> {teacher.phone} </td>
					<td>
						<button class="fc" type="button" on:click={()=>copyToClipboard(teacher.expand?.user_id.email)}>
							...
							{teacher.expand ? teacher.expand.user_id.email : '-'} 
						</button>
					</td>
					<td><div class="cell-fc"> {teacher.address || 'No consigna'} </div></td>
					<td>
						<div class="cell-fc">
							<a href="/">
								<svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
